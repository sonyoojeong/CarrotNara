...
public class QnaDao extends SuperDao{
	public int GetReplyCount(int groupno) throws Exception{
		...
		String sql = " select count(*) as cnt from Qna where groupno = ?" ;
	...
	public int ReplyData(Qna bean, int orderno) throws Exception{
		...
		String sql1 = " update qna set orderno = orderno + 1" ;
		...
		String sql2 = " insert into qna(no, writer, subject, content, regdate, groupno, orderno, depth)" ;
		sql2 += " values(myqna.nextval, ?, ?, ?, ?, ?, ?, ?)" ;
	...
	public int InsertData(Qna bean) throws Exception{
		System.out.println(bean);
		
		// bean 객체 1건의 데이터를 데이터 베이스에 추가합니다.
		int cnt = -1 ;
		
		String sql = " insert into qna(no, writer, subject, content, regdate, readhit, remark, groupno, orderno, depth)" ;
		sql += " values(myqna.nextval, ?, ?, ?, ?, default, ?, myQna.currval, default, default)" ;
	...
	// 게시물 1건 
	public Qna getDataByPk01(int no) {
		return new Qna(10, "hong", "자바 프로그램", "잼있어요", "2002/08/15", 1234, "비고란", 111, 222, 2);		
	} 
	
	public List<Qna> getDataList(){
		List<Qna> datalist = new ArrayList<Qna>();
		
		datalist.add(new Qna(10, "hong", "자바 프로그램", "잼있어요", "2002/08/15", 30, "비고란", 111, 222, 1));
		datalist.add(new Qna(20, "hong", "파이썬 프로그램", "잼있어요", "2002/08/15", 40, "비고란", 111, 222, 1));
		datalist.add(new Qna(30, "hong", "database", "잼있어요", "2002/08/15", 50, "비고란", 111, 222, 1));
		datalist.add(new Qna(40, "park", "자바 프로그램", "잼있어요", "2002/08/15", 60, "비고란", 111, 222, 2));
		datalist.add(new Qna(50, "park", "자바 프로그램", "잼있어요", "2002/08/15", 70, "비고란", 111, 222, 2));
		
		
		return datalist ; 
	}
	
	public List<Qna> SelectAll(Paging pageInfo) throws Exception{
	...
	private Qna makeBean(ResultSet rs) throws Exception {
		Qna bean = new Qna() ;
	...
	public Qna getDataByPk(int no) throws Exception {
	...
	public int UpdateData(Qna bean) throws Exception{
		// 게시물 작성자가 게시글을 수정합니다.
		System.out.println(bean);
		int cnt = -1 ;
	...
	public int UpdateReadhit(int no) throws Exception{
		...
		// remark 컬럼은 관리자가 상품 삭제시 자동으로 업데이트 됩니다.
		String sql = " update qna set readhit = readhit + 1 where no = ?" ; 

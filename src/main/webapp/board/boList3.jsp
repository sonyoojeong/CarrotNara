...
<body>
	...
	<div class="container mt-3">
		...
		<table class="table">
			...
			<tbody>
				...
				<tr>
					...
					<td>${bean.writer}</td>
					<td>
						<a id="noUnderLine" href="<%=notWithFormTag%>boDetail3&no=${bean.no}${requestScope.pageInfo.flowParameter}">						
					...
					<td>${bean.content}</td>
					<td>
					<img src="<%=contextPath%>/image/${bean.image01}" width="100px" height="100px"  class="card-img-top small_image" alt="${bean.no}">
					</td>
					...
					<td>
						...
						<c:if test="${sessionScope.loginfo.id==bean.writer}"> 
							<a href="<%=notWithFormTag%>boUpdate3&no=${bean.no}${requestScope.pageInfo.flowParameter}">
					...
					<td>
						 ...
						 <c:if test="${sessionScope.loginfo.id==bean.writer or whologin==2}"> 
							...
							onclick="if(confirm('삭제하시면 복구할 수 없습니다. \n 정말로 삭제하시겠습니까?')==true){
								location.href = '<%=notWithFormTag%>boDelete3&no=${bean.no}${requestScope.pageInfo.flowParameter}' ;}
		...
		<tr>
						...
						<a href="<%=notWithFormTag%>boRegister3">

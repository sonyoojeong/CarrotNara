...
<body>
	...
	<div class="container mt-3">
		...
		<table class="table">
			...
			<tbody>
				...
				<tr>
					...
					<td>${bean.writer}</td>
					<td>
						<a id="noUnderLine" href="<%=notWithFormTag%>boDetail5&no=${bean.no}${requestScope.pageInfo.flowParameter}">						
					...
					<td>
						...
						<c:if test="${whologin==2}"> 
							<a href="<%=notWithFormTag%>boUpdate5&no=${bean.no}${requestScope.pageInfo.flowParameter}">
					...
					<td>
						 ...
						 <c:if test="${whologin==2}"> 
							...
							onclick="if(confirm('삭제하시면 복구할 수 없습니다. \n 정말로 삭제하시겠습니까?')==true){
								location.href = '<%=notWithFormTag%>boDelete5&no=${bean.no}${requestScope.pageInfo.flowParameter}' ;}
		...
		<c:if test="${whologin==2}">
						...
						<a href="<%=notWithFormTag%>boRegister5">
